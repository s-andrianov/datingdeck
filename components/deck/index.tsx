...
export const DeckCards = ({
  ...
  return (
    <div
      className={cn("relative z-10 w-full max-w-[512px] mx-auto h-full", className)}
      style={{ perspective }}
      {...props}
    >
...
const DeckCard = ({
  ...
  return (
    <motion.div
      animate={exitDirection ? { x: exitX, rotate: exitRotate, opacity: 0 } : undefined}
    ...
    >
      {cloneElement(castedChildren, {
        className: cn(
          "h-full w-full select-none rounded-[40px] shadow-lg relative p-[10px]",
      ...
      <motion.div
        ...
        style={{
          background: "linear-gradient(270deg, rgba(255, 148, 185, 0) 0%, #FF94B9 83.65%)",
      ...
      <motion.div
        className="absolute inset-0 rounded-[40px] pointer-events-none"
        style={{
          background: "linear-gradient(270deg, #94FFAD 16.35%, rgba(148, 255, 173, 0) 100%)",
...
export const DeckItem = ({ className, ...props }: DeckItemProps) => (
  <div
    className={cn(
      "flex h-full w-full items-center justify-center rounded-[40px] border bg-card text-card-foreground shadow-lg",
      className
    )}
    {...props}
  />
...
export const DeckEmpty = ({
  ...
  <div
    ...
    {...props}
  >
    {children ?? <p className="text-sm">Больше нет анкет</p>}
  </div>
);
